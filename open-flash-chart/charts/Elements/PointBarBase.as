package charts.Elements {
	...
	public class PointBarBase extends Element
	{
		...
		public function PointBarBase( index:Number, value:Object, colour:Number, tooltip:String, group:Number )
		{
			...
			this.alpha = CONFIG::mouse_out_alpha;
		...
		protected function parse_value( value:Object ):void {
			if (value == null)
			 throw new Error("Null value specified.");
		...
		public override function mouseOut(event:Event):void {
			this.is_tip = false;
			Tweener.addTween(this, { alpha:CONFIG::mouse_out_alpha, time:0.8, transition:Equations.easeOutElastic } );
		}
		...
		protected function resize_helper( sc:ScreenCoords, axis:Number ):Object {
			...
			// If a bar is less than two pixels tall, don't try to draw it. The drawing
			// routine can't handle that small of a bar without causing strange fragments.
			if (Math.abs(bar_bottom - bar_top) < 2)
			  bar_top = bar_bottom;
