package charts {
	...
	import charts.Elements.PieLegend;
...
//			this.Elements= new Array();
			...
			if (this.style.legend) {
			  this.legend = new PieLegend(this.values, this.colours, this.style['label-style']);
		...
		private function add_slice( index:Number, start:Number, angle:Number, value:Object, tip:String, colour:String ): PieSliceContainer {
			...
			var default_style:Object = {
					...
					'label-style':  this.style['label-style'],
		...
		public override function resize( sc:ScreenCoordsBase ): void {
		  // XXX Change if shadow filter is changed.
		  var shadowSpace:Number = 8;
		  var marginLeft:Number = 3;
		  var effectiveWidth:Number = sc.width - shadowSpace - marginLeft;
		  var effectiveHeight:Number = sc.height - this.y - shadowSpace;
		  
		  if (legend != null) {
			  legend.x = sc.width - legend.get_width();
		    effectiveWidth = legend.x - 10 - shadowSpace - marginLeft;
		  }

			var radius:Number = (Math.min(effectiveWidth, effectiveHeight) / 2.0);
      
      if (legend != null) {
        legend.y = (sc.get_center_y() + radius) - legend.height;
      }
	  
			var pie:PieSliceContainer;
			if (this.legend == null) {
			  // If legend is on, we don't use outside labels so resizing is not necessary.
  			...
  			do
  			{
  				...
  				{
  				  // YMO Skip the legend.
  				  if (!(this.getChildAt(i) is PieSliceContainer))
  				    continue;
			...
			for ( i = 0; i < this.numChildren; i++ )
			{
			  ...
			  if (!(this.getChildAt(i) is PieSliceContainer))
				...
				pie = this.getChildAt(i) as PieSliceContainer;
				// YMO Make sure the pie is centered in the space to the left of the legend, when the legend
				// is on, center everywhere otherwise.
				pie.x = marginLeft + effectiveWidth / 2.0;
