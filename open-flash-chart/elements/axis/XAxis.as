package elements.axis {
	...
	public class XAxis extends Sprite {
		...
		public function resize( sc:ScreenCoords ):void
		{
			...
			{
				// YMO Change. Don't use drawRect, it causes fuzzy lines.
			  var active_colour:Number = this.grid_colour;
				if( ( this.alt_axis_step > 1 ) && ( i % this.alt_axis_step == 0 ) )
				  active_colour = this.alt_axis_colour;

				var x:Number = sc.get_x_from_val(i);
        this.graphics.lineStyle(1, active_colour, 1, true, LineScaleMode.NORMAL,
                                CapsStyle.ROUND, JointStyle.MITER);
			  this.graphics.moveTo(x, sc.top);
			  this.graphics.lineTo(x, sc.top + sc.height);
			}
		...
		public function two_d_axis( sc:ScreenCoords ):void
		{
		  if (this.stroke == 0)
			...
			// YMO Change: Use lineTo instead of draw rect.
			// Oddly this fixes a strange bug:
			// If the old drawRect code is used to draw the axis, the tooltip leaves
			// ghost traces on the right side of the graph when moved away. 
			this.graphics.lineStyle(this.stroke, this.colour, 1, true, LineScaleMode.NORMAL,
                              CapsStyle.ROUND, JointStyle.MITER);
		  this.graphics.moveTo(sc.left, sc.bottom);
		  this.graphics.lineTo(sc.left + sc.width, sc.bottom);
						
			CONFIG::debug { tr.ace('gc'); }
			CONFIG::debug { tr.ace(this.style.max); }
			CONFIG::debug { tr.ace(this.steps); }
			for( var i:Number=this.style.min; i <= this.style.max; i+=this.steps )
			{
				var x:Number = sc.get_x_from_val(i);
				this.graphics.beginFill(this.colour, 1);
